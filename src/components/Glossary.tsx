...
type Category = 'how' | 'players' | 'match' | 'simulation';

const CATEGORIES: { key: Category; label: string }[] = [
  { key: 'how', label: 'How It Works' },
  { key: 'players', label: 'Player Stats' },
  { key: 'match', label: 'Match Tools' },
  { key: 'simulation', label: 'Simulation' },
];

export default function Glossary() {
  const [showGlossary, setShowGlossary] = useState(false);
  const [activeCategory, setActiveCategory] = useState<Category>('how');
    ...
    <div className="mt-8">
      ...
      <AnimatePresence>
        {showGlossary && (
          ...
          >
            <div className="bg-surface-card rounded-card shadow-card p-4 md:p-6 mt-2 text-sm">
              {/* Category pills */}
                  ...
                  <button
                    ...
                    className={clsx(
                      ...
                      activeCategory === cat.key
                        ...
                        : 'bg-surface text-gray-400 hover:text-white hover:bg-surface-elevated'
                    )}
                  >
                    {cat.label}
                  </button>
              ...
              {activeCategory === 'how' && (
                <div className="space-y-5">
                  <div>
                    <h3 className="font-bold text-info mb-1">The Prediction Model</h3>
                    <p className="text-gray-400">
                      ...
                      uses a <span className="text-white">statistical model</span> to convert the strength
                  ...
                  <div>
                    <h3 className="font-bold text-info mb-1">Season Simulation (Monte Carlo)</h3>
                    <p className="text-gray-400">
                  ...
                  <div>
                    <h3 className="font-bold text-info mb-1">Points System</h3>
                    <p className="text-gray-400">
                  ...
                  <div>
                    <h3 className="font-bold text-info mb-1">Confidence-Adjusted Win% (Adj%)</h3>
                    <p className="text-gray-400">
                      ...
                      <span className="text-white">Bayesian regression toward 50%</span> so
                      that small samples are pulled toward the mean while large samples stay mostly unchanged.
                      All rankings, squad strength calculations, and lineup suggestions use adjusted percentages.
                    </p>
                    <button
                      onClick={() => setShowFormula(!showFormula)}
                      className="text-xs text-accent-light hover:text-accent transition mt-1"
                    >
                      {showFormula ? 'Hide formula' : 'Show formula'}
                    </button>
                    {showFormula && (
                      <div className="mt-2 text-xs text-gray-300 font-mono bg-surface/50 rounded p-2">
                        adj = (wins + 6 &times; 0.5) / (games + 6)
                      </div>
                    )}
              ...
              {activeCategory === 'players' && (
                <div className="space-y-3">
                  <div>
                    <div className="font-medium text-info">Last Season Rating</div>
                    <p className="text-gray-400 text-xs">
                      A performance rating carried over from last season&apos;s data. Green positive values
                      indicate above-average players; red negative values indicate below-average. This provides
                      historical context for players with limited current-season games.
                    </p>
                  </div>
                  <div>
                    <div className="font-medium text-info">Current Win%</div>
                    <p className="text-gray-400 text-xs">
                      The percentage of frames a player has won this season. Calculated as{' '}
                      <span className="text-gray-300">frames won / frames played &times; 100</span>.
                    </p>
                  </div>
                  <div>
              ...
              {activeCategory === 'simulation' && (
                <div className="space-y-3">
                  ...
                  <div>
                    <div className="font-medium text-info">Predicted Points (Pred)</div>
                    <p className="text-gray-400 text-xs">
